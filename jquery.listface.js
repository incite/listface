(function($) {
  ...
  $.listface = function(selector, options) {
      ...
      textField.keydown(function(event) {
        switch(event.keyCode) {
          ...
          case KEY.BACKSPACE:
            ...
            // fall through (backspace widens search)
          default:
            clearFocus();
    ...
    function enforceMaxEntries() {
      var activeEntryCount = list.find('li.selected').length;
      if (options.maxEntries) {
        if (activeEntryCount >= options.maxEntries) {
    ...
    function add(item) {
      ...
      if(options.addItem) {
        ...
        var id   = item.find('span:last').text();
        options.addItem(item, id, name);
      }
    }
    ...
    function remove(item) {
      ...
      if(options.removeItem) {
        var name = item.find('span:first').text();
        var id   = item.find('span:last').text();
        options.removeItem(item, id, name);
      }
    }
          ...
          if (options.attribute) {
              li = $([
                '<li>',
                  ...
                  '<span style="display: none">', item[options.attribute.value], '</span>',
                '</li>'
              ].join(''));
          } else {
    ...
    function verify(options) {
      ...
      if (options.addItem && options.addItem.constructor != Function)         { throw "addItem needs to be a Function" }
      if (options.removeItem && options.removeItem.constructor != Function)   { throw "addItem needs to be a Function" }
